 ...
 postDBLoad(container) {
  ...
  const funni =
   [
    ...
    "What?", "Really, stop looking here, play the game already >:C",
    ...
    if (i !== "base" && locations[i].base.BossLocationSpawn) {
     for (let x in locations[i].base.BossLocationSpawn) {
      switch (locations[i].base.BossLocationSpawn[x].BossName) {
       ...
       case "bossKolontay":
        if (i == "sandbox_high") {
         locations[i].base.BossLocationSpawn[x].BossChance = Config.Bots.AIChance.GZKolontay
        }
        if (i == "tarkovstreets") {
  ...
  if (Config.CSM.EnableCSM) {
   if (Config.CSM.CustomPocket) {
    ...
    CustomPocketItem._props.Grids[1]._parent = "a8edfb0bce53d103d3f62b9b"
    ...
    CustomPocketItem._props.Grids[2]._parent = "a8edfb0bce53d103d3f62b9b"
    ...
    CustomPocketItem._props.Grids[3]._parent = "a8edfb0bce53d103d3f62b9b"
    ...
    CustomPocketItem._props.Slots[0]._parent = "a8edfb0bce53d103d3f62b9b"
    CustomPocketItem._props.Slots[1]._parent = "a8edfb0bce53d103d3f62b9b"
    CustomPocketItem._props.Slots[2]._parent = "a8edfb0bce53d103d3f62b9b"
    ...
    switch (Config.CSM.Pockets.SpecSlots) {
     ...
     case 4://This need to be fixed, i need to make it gradual for each additional pocket rather using break, will be fixed in future.
      ...
      // 3.9.0 Attempt to avoid removal of special slot items
   ...
   for (let i in locations) {
    if (i !== "base") {
     for (let x in locations[i].base.exits) {
      if (locations[i].base.exits[x].Name !== "EXFIL_Train" && (!locations[i].base.exits[x].Name.includes("lab") || locations[i].base.exits[x].Name == "lab_Vent") && locations[i].base.exits[x].Name !== "Saferoom Exfil") {//Ok, i feel dumb again, but i was in a rush ok?
  ...
  //  }
   ...
   if (Config.Raids.RaidEvents.BossesOnHealthResort) {
    ...
    if (Config.Raids.RaidEvents.HealthResortIncludeGuards) {
     ...
     locations["shoreline"].base.BossLocationSpawn.push(Kolontay)
    }
    else {
     let BossWave = CreateBoss("bossKilla", 100, "followerBully", 0, "ZoneSanatorium1,ZoneSanatorium2")
     locations["shoreline"].base.BossLocationSpawn.push(BossWave)
  ...
  if (Config.PMC.EnablePMC) {
   PMC.convertIntoPmcChance.assault.min = Config.PMC.AItoPMC.PMCtoScav; //Yes, it should be ScavtoPMC, don't ask, same to raider and cursed.
  ...
  if (Config.Scav.EnableScav) {
   ...
   //PMC kill rep
   Bot["pmcbear"].experience.standingForKill = Config.Scav.StandingPMCKill;
   Bot["pmcusec"].experience.standingForKill = Config.Scav.StandingPMCKill;
